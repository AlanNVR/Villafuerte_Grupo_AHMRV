...
% =====================================================================
\newcommand{\RF}[8]{%
\begin{longtable}{|L{3.2cm}|L{12.2cm}|}
\hline
\rowcolor{grisclaro}\multicolumn{2}{|l|}{\textbf{#1 --- #2}}\\
\hline
...
\url{https://github.com/AlanNVR/Villafuerte_Grupo_AHMRV}
...
3 & Ausencia de fotografías de la aplicación real del cuestionario. & Incorporadas en \id{02\_Evidencias/Cuestionario/Fotos\_Aplicacion/}, con rostros fuera de encuadre. \\ \hline
...
\begin{figure}[htbp]
\centering
% \includegraphics[width=0.95\textwidth]{img/diagrama_contexto.png}
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm} [Insertar \id{img/diagrama\_contexto.png}] \vspace{2cm}}}
...
Jefe de polinización & Medio & Alto & Mantener informado & \id{EV-03} \\ \hline
...
Agrocalidad (ente regulador) & Alto & Bajo & Monitorear & \id{EV-04} \\ \hline
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm} [Insertar \id{img/istar\_sd.png}] \vspace{2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm} [Insertar \id{img/istar\_sr.png}] \vspace{2cm}}}
...
\subsection{Resultados del cuestionario aplicado ($n=62$)}
...
Esta sección especifica 35 requisitos funcionales, 18 requisitos no funcionales cuantificados sobre
...
\begin{longtable}{|C{1.5cm}|L{2.6cm}|L{6.2cm}|L{3.4cm}|C{1.3cm}|}
\hline
...
\hline
\endhead
\id{RNF-01} & Adecuación funcional & La clasificación de madurez del racimo alcanza una exactitud $\geq 85\%$ y una exhaustividad $\geq 80\%$ sobre la clase ``verde'' en el conjunto de prueba. & Matriz de confusión sobre conjunto etiquetado independiente de $\geq 200$ imágenes. & \id{EV-04} \\ \hline
\id{RNF-02} & Adecuación funcional & La detección de plagas alcanza una exactitud $\geq 80\%$ sobre las cinco afectaciones más frecuentes del cultivo. & Evaluación sobre conjunto etiquetado por la persona asesora técnica. & \id{EV-01}, \id{EV-02} \\ \hline
\id{RNF-03} & Eficiencia de desempeño & Las operaciones de registro y consulta responden en $\leq 3$ s para el percentil 95, con hasta 50 sesiones concurrentes. & Prueba de carga con 50 usuarios virtuales. & Derivado \\ \hline
\id{RNF-04} & Eficiencia de desempeño & El diagnóstico por imagen entrega resultado en $\leq 10$ s con ancho de banda $\geq 5$ Mbps. & Tiempo medio sobre 20 análisis consecutivos. & \id{EV-01}, \id{EV-02} \\ \hline
\id{RNF-05} & Eficiencia de desempeño & El registro GPS captura la ubicación con precisión $\leq 10$ m y período de muestreo $\leq 30$ s. & Contraste de 10 puntos con referencia geodésica. & \id{EV-03} \\ \hline
\id{RNF-06} & Compatibilidad & La exportación hacia la planta extractora se emite en formato CSV o JSON conforme al esquema documentado, sin campos de identificación personal. & Validación del archivo exportado contra el esquema publicado. & \id{EV-04} \\ \hline
\id{RNF-07} & Compatibilidad & La aplicación móvil opera en Android 9.0 o superior; el panel web, en las dos últimas versiones estables de Chrome, Edge y Firefox. & Instalación y ejecución sin error en los entornos declarados. & Derivado \\ \hline
...
\id{RNF-10} & Fiabilidad & El servicio de respaldo presenta una disponibilidad mensual $\geq 99\%$, equivalente a una indisponibilidad $\leq 7$ h 18 min. & Monitoreo de disponibilidad durante 30 días. & Derivado \\ \hline
...
\id{RNF-12} & Fiabilidad & Ante pérdida de señal GPS, el sistema conserva el registro parcial y lo marca como de precisión degradada, sin descartar la jornada. & Simulación de pérdida de señal durante una jornada de polinización. & \id{EV-03} \\ \hline
\id{RNF-13} & Seguridad & Las credenciales se almacenan mediante función de derivación de clave con sal; la totalidad del tráfico emplea TLS 1.2 o superior. & Inspección del almacén de credenciales y análisis del tráfico. & Derivado \\ \hline
\id{RNF-14} & Seguridad & Los datos de geolocalización se cifran en reposo y su acceso queda registrado en bitácora con identificación de la persona usuaria, fecha y motivo. & Auditoría de la bitácora tras 10 accesos de prueba. & \id{lopdp} Art. 37 \\ \hline
\id{RNF-15} & Mantenibilidad & El sistema presenta arquitectura modular con cobertura de pruebas automatizadas $\geq 60\%$ en la capa de lógica de negocio. & Reporte de cobertura de la herramienta de integración continua. & Derivado \\ \hline
\rowcolor{grisclaro}\id{RNF-16} & Interacción de usuario & \textbf{Explicabilidad} --- véase la especificación ampliada en la §3.3.1. & Prueba con 6 participantes (3 técnicos, 3 no técnicos). & \id{EV-01}, \id{EV-02}, \id{EV-07} \\ \hline
\id{RNF-17} & Portabilidad & La migración del servicio de inferencia a un proveedor alternativo no requiere modificar el cliente móvil, al mediar una interfaz de servicio desacoplada. & Sustitución del proveedor en entorno de prueba sin cambios en el cliente. & Derivado \\ \hline
\id{RNF-18} & Flexibilidad & Los umbrales de madurez, de porcentaje de fruta verde y de ventana corte-entrega son parametrizables por variedad y por lote sin recompilar la aplicación. & Modificación de los tres umbrales desde la configuración y verificación de su efecto. & \id{EV-04} \\ \hline
